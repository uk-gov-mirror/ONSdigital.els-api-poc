...
<fieldset>
  {#if label}<legend>{label} <strong>{formatTick(selectedRange[0])}</strong> to <strong>{formatTick(selectedRange[1])}</strong></legend>{/if}
  <div class="range-container">
    <div class="range-ticks">
      {#each options as tick, i}
        <div class="range-tick" class:range-tick-end={i === 0 || i === options.length - 1} style:left="{scale(i)}%"></div>
      {/each}
      {#each [0, options.length - 1] as i}
        <div class="range-tick-text" style={i === 0 ? "left: 0" : "right: 0"}>{formatTick(options[i])}</div>
      {/each}
    </div>
